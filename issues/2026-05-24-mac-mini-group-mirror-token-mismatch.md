...
5. `bash -x` trace 로 stderr 살림 → **token 값 노출**: `8618050273:AAGY_pXqJTE4bseo2IfMMKcMpb27Ct5ygYE`
6. 본진 SoT 의 TOKEN_MACMINI = `8618050273:AAFtQwR4bRA2fmtKhUaJIYtMx4v30WDpSx0` 와 **secret 부분 다름**
...
```bash
ssh mac-mini "cp ~/.claude/channels/telegram/.env ~/.claude/channels/telegram/.env.bak-pre-tokenfix-$(date +%Y%m%d-%H%M%S) && sed -i.tmp 's|^TELEGRAM_BOT_TOKEN_MACMINI=.*|TELEGRAM_BOT_TOKEN_MACMINI=8618050273:AAFtQwR4bRA2fmtKhUaJIYtMx4v30WDpSx0|' ~/.claude/channels/telegram/.env && rm -f ~/.claude/channels/telegram/.env.tmp"
```

...
Synced: 2026-05-15T18:34:55
-->

# 거절을 큐에서 다시 돌리는 법

_바이브코딩 뉴스레터 Ep.5 — 한줄일기 1.0 출시기, 거절과 승인 사이 30시간_
...
4월 29일 06시 43분 KST. Apple 메일이 도착했다. 제목은 “There’s an issue with your 한줄일기 (iOS) submission”. reviewSubmission state 는 **UNRESOLVED\_ISSUES**. 사유는 Guideline 5 — Legal.
...
ASC API 로 직접 들여다봤다. `GET /v1/reviewSubmissions/7469efbf...` 의 결과는 그대로 **UNRESOLVED\_ISSUES**. Apple 이 매뉴얼 액션을 안 한 것이다. 옛 reviewSubmission 이 appStoreVersion 을 점유한 채 잠겨있었고, 새 reviewSubmission `e58db9c9` 한 개가 자동 생성돼있었지만 **items=0 빈 껍데기** 였다.

정공법으로 풀어보려 했더니 모두 막혀있었다.

* POST 새 sub 에 appStoreVersion attach → **409 ITEM\_PART\_OF\_ANOTHER\_SUBMISSION** (옛 sub 가 점유 중이라 못 붙임)
...
11시 6분 KST, 새 sub 가 WAITING\_FOR\_REVIEW 로 큐에 다시 들어갔다. **답글을 기다리지 않은 결정의 시각.**
...
* **구간**: 첫 제출 / **시각**: 4/28 23:04 KST / **소요**: —
* **구간**: 거절 메일 / **시각**: 4/29 06:43 KST / **소요**: ~7시간
* **구간**: 답글 송신 / **시각**: 4/29 08:05 KST / **소요**: +1시간
* **구간**: 답글 무응답 / **시각**: 4/29 08:05 ~ 4/30 11:06 / **소요**: **27시간**
* **구간**: 재제출 (PATCH canceled=true) / **시각**: 4/30 11:06 KST / **소요**: —
* **구간**: **승인 메일** / **시각**: **4/30 13:29 KST** / **소요**: **2시간 23분**

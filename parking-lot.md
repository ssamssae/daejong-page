...
- [ ] 🍎 🎰 lotto-calc dhlottery 자동 감지 cron (Mac mini launchd) — 매일 1회(예: 09:00 KST) `curl gameResult.do?method=byWin&drwNo=N` 응답 본문에 `drwtNo[1-6]` 또는 `<th>당첨번호</th>` 토큰 발견 시 PASS → 텔레그램 알림 1통 ("dhlottery 정상 복구, lotto-calc v1 시드 GO 사이클 진입 가능"). 토큰 0 = silent skip. 강대종 손 0 + 복구 시점 즉시 알림 + lotto-calc v1 시드 사이클 즉시 진입 가능. 자동화 자체 ~30분 작업(cron + curl + 토큰 grep + 텔레그램 reply). 트리거 = lotto-calc 우선순위 올리거나 dhlottery 복구 늦어 진행 압박 생길 때.  (추가: 2026-05-02 WSL session-close 후속안 → session-close abc 박아 결정)

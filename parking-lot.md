...
- 약먹자 v1.0.3+ personalized 광고 사이클 — 이번 1.0.2+5 출시(2026-05-12)는 ATT 키 제거 + AdMob non-personalized fallback 로 좁혔음. 다음 사이클: (1) Info.plist `NSUserTrackingUsageDescription` 다시 박기 (적절한 사용자 메시지) (2) ASC App Privacy 섹션 → Data Types 선언 (IDFA / 제3자 광고 / 추적함=Yes) (3) AdMob `AdRequest(nonPersonalizedAds: false)` 또는 ATT 동의 후 personalized — 셋 다 set 으로 박혀야 reviewSubmissionItems STATE_ERROR.BINARY_INDICATES_APP_TRACKS_USERS 함정 회피. (2026-05-12)
- iOS Info.plist `ITSAppUsesNonExemptEncryption=false` 다른 Flutter 앱들에 일괄 박기 — 더치페이/약먹자에는 2026-05-12 박았음 (영구 fix). 나머지 한줄일기/한컵/포모도로/단어요/로또계산기/메모요/주식모니터/AI응원일기 등 iOS 출시 앱 전수 점검 + 1줄 추가 + 다음 build 부터 ASC dialog 우회. lesson: `~/.claude/skills/submit-app/lessons/ios-asc-uses-non-exempt-encryption.md` (2026-05-12)
- asc-submit.py / asc-resubmit-v2.py 정식 스크립트화 — 현재 `/tmp/` 위치라 휘발성. `~/.claude/automations/scripts/asc-submit.py` (신규 버전 ASC 제출) / `~/.claude/automations/scripts/asc-resubmit-v2.py` (rejected build 재제출, Python 3.9 호환) 로 이전 + asc-deliver.py 의 Python 3.10+ syntax 마이그레이션 또는 deprecation 결정. 함정 4종 (usesNonExemptEncryption 'already set' 409 skip, BINARY_INDICATES_APP_TRACKS_USERS, version state propagation 5s × 12 retry, timeout 90s) 내장. submit-app 스킬에서 호출 가능하게 wiring 필요. (2026-05-12)
- araseo-voice live mic + heyclaude.ppn smoke (Mac 본진, PR #8 머지 후) — Task 5 본체는 WSL/Hermes 풀그린(pytest 53 pass + lazy import 검증). 환경 의존 post-merge validation: (1) PV_ACCESS_KEY 환경변수 (2) `~/.araseo/voice/heyclaude.ppn` 모델 파일 (3) macOS mic 권한 (4) 짧은 wake → capture → STT 라운드 1회. PR #8 머지 후 본진에서 1회 smoke → 통과 표기. (2026-05-12)

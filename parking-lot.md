...
<!-- 2026-05-03 — AI 응원 앱 scaffold 완료 (ssamssae/ai_cheer, ai_cheer PR #1 MERGED), 주식 모니터링 앱 scaffold 완료 (ssamssae/stock_monitor, stock_monitor PR #1 MERGED), /stack.html SVG PR #36 MERGED, Plan C 발행 완료 (Substack + 네이버), 이슈→노하우 PR #37 3건 MERGED → 5개 항목 정리 -->
...
- [ ] 🤝 💡 이슈 → 노하우 측면 강한 거 추가 이전 — 2026-05-03 PR #37 로 3건 이전 완료 (interrupt-git-guard, macos-ssh-fda, cf-workers-region-block). 잔여 후보: 4/17 시뮬레이터 SharedPrefs cfprefsd 캐시, 4/17 시뮬레이터 좌표 drift, 4/26 mac-ssh-stale-socket, 4/15 telegram-typing-daemon-orphan, 4/24 playwright-mcp-misclick. 사건 측면 약하고 재사용 패턴 강한 것만 추려 노하우로 일반화 + 이전. 트리거 = goodnight 또는 별도 사이클, 시간 날 때.  (추가: 2026-05-02 session-close)
...
- [x] 🤝 📊 lotto-calc StatsScreen 차트 시각화 (fl_chart 도입) — 현재 StatsScreen v1 (PR #6 머지 LIVE) 이 빈도 Top10 / 최근 Top5 / 미출현 Top5 ListView 만. fl_chart 도입해 빈도 막대그래프 + 최근/미출현 시각화. 통계 알고리즘은 기존 lotto_stats.dart 그대로 재사용. 트리거 = lotto-calc 출시 직전 또는 강대종 시각 검증 욕구 발생 시.  (추가: 2026-05-03 session-close ab 박음, 완료: 2026-05-03 — lottocalc bcdf860 S4/S5/S6 + 7fd91d8 fl_chart BarChart)
- [x] 🤝 📊 lotto-calc 통계 알고리즘 S4+ (짝홀 비율 / 합계 분포 / 연속 번호 빈도) — 현재 lotto_stats.dart S1/S2/S3 (frequency / recent / drought) 만. 강대종 통계 가설(1222 라운드 baseline 추첨) 강화에 필수. dogfood 기준으로 어떤 지표가 실제 의미 있는지 검증 후 단계적 추가. 트리거 = StatsScreen 차트 작업과 묶거나 별도 사이클.  (추가: 2026-05-03 session-close ab 박음, 완료: 2026-05-03 — lottocalc bcdf860 S4/S5/S6 + 7fd91d8 fl_chart BarChart)

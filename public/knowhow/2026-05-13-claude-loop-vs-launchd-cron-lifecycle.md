...
## 환경 기준 적합 케이스 분류 (강대종 환경)

### dynamic loop 에 어울리는 작업 — "짧게 돌고 끝남"

1. **앱 빌드 기다리기** — iOS/Android 빌드 시작 → TestFlight·Play Console 처리중? 5분 뒤 재확인 → 완료되면 알림
2. **Claude/Codex 작업 감시** — 에이전트가 코드 수정 → 테스트 실행 → 에러 로그 확인 → 재수정 → 성공하면 종료
3. **GitHub Actions 완료 polling** — CI 상태 확인 → running → 30초 후 재확인 → success
4. **배포 완료 대기** — 서버 deploy 요청 → healthcheck polling → 서비스 정상 확인 → 종료

공통 특징:
- 몇 분 ~ 1시간 내 종료
- 모델이 계속 생각하면서 진행 가능
- 세션 유지 가능
- 매 iter 마다 결과 보고 직접 사람에게 surface 가능

### launchd / systemd / fleet 에 어울리는 작업 — "영구 서비스"

1. **Telegram relay bot** — 24시간 메시지 대기 → 명령 수신 → worker 전달 → 응답 relay. 세션 끊기면 안 됨 = daemon.
2. **자동 인스타 업로드** — 매일 오전 9시 실행 → 이미지 생성 → 업로드. cron/launchd.
3. **watchdog** — 프로세스 죽었는지 감시 → 죽으면 자동 재시작. OS 레벨 서비스.
4. **worker mesh** — mission queue 감시 → 새 mission claim → worker 실행 → 결과 보고. daemon 구조가 안정적.
...
판단 기준:
- **시간 스케일** — 1시간 이하 = loop, 1일 이상 = daemon
- **컨텍스트 필요성** — 매 fire 가 이전 결과 봐야 함 = loop, 독립적 = daemon
- **사용자 surface** — 결과를 사람이 봐야 함 = loop (대화 흐름 안), 자동 처리만 = daemon
- **세션 의존 가능 여부** — 항상 켜져있어야 함 = daemon (세션 꺼지면 죽으면 안 됨)
...
- fleet-director — 멀티 머신 분산 워크 (D07 까지 완료, D08 launchd 등록 ack 대기)

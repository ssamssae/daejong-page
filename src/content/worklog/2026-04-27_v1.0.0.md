...
> v3 노드 정책의 두 가정(MacBook 이동성 + WSL 24/7 ON)이 강대종님 직접 정정으로 무너지면서 옵션 X 확정. 같은 흐름에서 trend-curator 풀제거, /stale 스킬 신설, 약먹자·더치페이 iOS reject 재대응 발송, [한줄일기](https://play.google.com/store/apps/details?id=com.daejongkang.hanjul) UI 마이크로 인터랙션 폴리시까지.
...
### 사이드 프로젝트 5→1 정리
- 단어요만 남김 (한줄일기 출시 후 큐)
- [메모요](https://play.google.com/store/apps/details?id=com.daejongkang.simple_memo_app)·심사레이더·바이브뉴스레터·한줄일기 등 운영 단계 진입한 것들은 사이드 카테고리에서 분리

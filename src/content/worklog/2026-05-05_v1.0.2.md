...
## rail-booking-sniff 스킬 신설 — mitmproxy iOS API 스니핑

### 21:27~21:58 KST — PR #24 머지 (feat(rail-booking-sniff): mitmproxy iOS API 스니핑 설정 스크립트 + 방법론 문서)

철도 예매 앱의 REST API를 역추적하는 새 접근법. Playwright 웹 우회 막힌(dead-end) 이후의 대안.

iOS 앱은 PerimeterX 봇탐지를 거치지 않는다. mitmproxy를 Mac과 iPhone 사이에 끼워 철도 예매 앱 트래픽을 캡처하면, `/api/train/getTrainSeatAvailability` 류의 실 엔드포인트와 세션 헤더를 확보할 수 있다.

- `setup_rail_booking_sniff.sh` — mitmproxy 자동 설치 + Mac Wi-Fi IP 감지 + 프록시 활성화
...
| claude-skills | 21a0146 | feat(rail-booking-sniff): mitmproxy iOS API 스니핑 설정 스크립트 + 방법론 문서 (#24) |
| claude-skills | 0dfffb3 | fix(rail-booking-sniff): Python 3.13 framework bin PATH 추가 |
| claude-skills | 0c6e1d3 | fix(rail-booking-sniff): Mac 호환 IP 검출 + mitmproxy PATH 자동탐지 |

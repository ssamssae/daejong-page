...
mac-mini 에 adb 미설치 사실 발견 (Android 빌드 엔진인데 platform-tools 누락 갭) → `brew install --cask android-platform-tools` 박음 (37.0.0). Galaxy S24 (SM_S921N, 테스트 계정) USB detect PASS. 폰의 Google 계정이 Test users 등록 계정과 일치라 Drive 권한 동의 시트 흐름은 그대로 검증 가능.
